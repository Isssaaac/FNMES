...
namespace FNMES.Utility.Network
{
    ...
    public class WebApiRequest
    {
        ...
        #endregion
        #region 异步方法
        public static RetMessage<T> DoGetAsync<T>(string url, Dictionary<string, string> parms, int? timeout = 3000) where T : new()
        {
            try
            {
                string ret = HttpUtils.DoGetAsync(url, parms, timeout);
                if (ret.IsNullOrEmpty())
                    return null;
                return ret.ToObject<RetMessage<T>>();
            }
