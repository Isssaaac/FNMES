...
layui.define(['element', 'common'], function (exports) {
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        if (tabIndex === -1) {
            ...
            title += '<cite style="-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;">' + data.title + '</cite>';
            if (_config.closed) {
                ...
                if ($context.length > 0) {
                    $context.eq(0).children('ul').children('li').each(function () {
                        ...
                        $that.on('click', function () {
                            ...
                            switch (target) {
                                case 'refresh': //刷新当前
                                    var iframe = ELEM.contentBox.find('iframe[data-id=' + id + ']');
                                    var origin = iframe.context.location.origin;
