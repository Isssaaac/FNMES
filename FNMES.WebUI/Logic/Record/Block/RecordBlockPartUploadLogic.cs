...
{
    public class RecordBlockPartUploadLogic : BaseLogic
    {
        ...
        public int Insert(PartUploadParam model, string configId)
        {
            try
            {
                RecordBlockPartUpload recordPartUpload = new RecordBlockPartUpload();
                ...
                List<RecordBlockPartData> partList = new();
                foreach (Part item in model.partList)
                {
                    RecordBlockPartData buf = new RecordBlockPartData();
                    buf.CopyField(item);
