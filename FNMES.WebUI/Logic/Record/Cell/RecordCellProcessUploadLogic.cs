...
{
    public class RecordCellProcessUploadLogic:BaseLogic
    {
        public int Insert(ProcessUploadParam model, string configId)
        {
            try
            {
                ...
                List<RecordCellProcessData> processList = new();
                foreach (var item in model.processData)
                {
                    RecordCellProcessData buf = new();
                    buf.CopyField(item);
                ...
                db.Insertable<RecordCellProcessUpload>(process).SplitTable().ExecuteCommand();
                db.Insertable<RecordCellProcessData>(processList).SplitTable().ExecuteCommand();
                Db.CommitTran();
                return 1;
            }
            catch (Exception e)
